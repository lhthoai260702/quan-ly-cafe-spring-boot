 ...
 * @param {string} valueStr - Chuỗi số cần định dạng
...
/**
 * Xử lý khi người dùng nhập dữ liệu vào ô Mức giảm
 * @param {HTMLElement} displayEl - Thẻ input hiển thị
 * @param {HTMLElement} hiddenEl - Thẻ input ẩn lưu giá trị thực
 * @param {HTMLElement} typeEl - Thẻ select chọn loại khuyến mãi
 */
function handleInputDiscount(displayEl, hiddenEl, typeEl) {
    let rawVal = displayEl.value.replace(/\D/g, '');

    if (typeEl.value === 'Phần trăm') {
        if (rawVal !== '' && parseInt(rawVal, 10) > 100) {
            rawVal = '100';
        }
        if (rawVal !== '' && parseInt(rawVal, 10) < 1) {
            rawVal = '1';
        }
...
/**
 * Cập nhật điều kiện kiểm tra hợp lệ dựa theo loại khuyến mãi (Phần trăm / Tiền mặt)
 * @param {HTMLElement} typeEl - Thẻ select chọn loại khuyến mãi
 * @param {HTMLElement} displayEl - Thẻ input hiển thị
 * @param {HTMLElement} hiddenEl - Thẻ input ẩn lưu giá trị thực
 */
function updatePromoValidation(typeEl, displayEl, hiddenEl) {
    ...
    if (typeEl.value === 'Phần trăm') {
        ...
        if (currentVal && parseInt(currentVal, 10) > 100) {
            currentVal = '100';
        }
...
/**
 * Mở hộp thoại (Modal) sửa thông tin chương trình khuyến mãi
 * @param {string|number} id - Mã khuyến mãi
 * @param {string} name - Tên chương trình
 * @param {string} start - Ngày bắt đầu
 * @param {string} end - Ngày kết thúc
 * @param {string} type - Loại khuyến mãi
 * @param {string|number} val - Giá trị giảm
...
/**
 * Mở hộp thoại (Modal) xác nhận xóa khuyến mãi
 * @param {string|number} id - Mã khuyến mãi
 * @param {string} name - Tên chương trình
 */
...
/**
 * Khóa ngày kết thúc tự động dựa trên ngày bắt đầu (Ngày kết thúc > Ngày bắt đầu)
 * @param {string} startId - ID của thẻ input chọn ngày bắt đầu
 * @param {string} endId - ID của thẻ input chọn ngày kết thúc
 */
    ...
    if (startEl && endEl) {
        startEl.addEventListener('change', function () {
            if (this.value) {
                ...
                d.setDate(d.getDate() + 1); // Ngày kết thúc min = Ngày bắt đầu + 1 ngày
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (addType && addValDisplay && addValHidden) {
        ...
        addValDisplay.addEventListener('input', function () {
            handleInputDiscount(addValDisplay, addValHidden, addType);
        });
    }
    ...
    if (editType && editValDisplay && editValHidden) {
        editType.addEventListener('change', function () {
            updatePromoValidation(editType, editValDisplay, editValHidden);
        });
        editValDisplay.addEventListener('input', function () {
            handleInputDiscount(editValDisplay, editValHidden, editType);
        });
    }
    ...
    forms.forEach(function (form) {
        ...
        // Tự động gỡ lỗi khi người dùng nhập lại
